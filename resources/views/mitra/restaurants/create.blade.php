...
@section('page-title', 'Restoran Baru')
@section('page-subtitle', 'Lengkapi informasi restoran Anda')

@section('content')

<div class="bg-white rounded-xl shadow-sm p-6">
    <form action="{{ route('mitra.restaurants.store') }}" method="POST" class="space-y-6" id="restaurant-form">
        @csrf
        ...
        <div>
            ...
            <select id="restaurant-city" class="input-control" data-static-options="true">
        ...
        <input type="hidden" name="city_id" id="restaurant-city-id" value="{{ old('city_id') }}">
        ...
        <div>
            ...
            <input type="text" name="address" id="restaurant-address" value="{{ old('address') }}" required class="input-control">
        </div>



        <div class="flex justify-between items-center">
            <a href="{{ route('mitra.restaurants.index') }}" class="px-4 py-2 rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50">Batal</a>
            <button class="px-6 py-2 rounded-lg bg-ocean-600 text-white hover:bg-ocean-700">Simpan Restoran</button>
        </div>
    </form>
</div>
@endsection
...
(function () {
    ...
    const cityHidden = document.getElementById('restaurant-city-id');
